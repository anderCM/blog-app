...
describe Comment, type: :model do
  it 'Should save a comment' do
    ...
    user_comment.save
    comment = Comment.new(user: user_comment, post: post, text: "Nice post @#{user.name}")
  ...
  it 'Should increment comment counter' do
    ...
    user.save
    post = Post.new(author: user, title: 'How to become a software developer', text: 'The first step is...', comment_counter: 0,
        like_counter: 0)
    post.save
    user_comment = User.new(name: 'Yensi Roff', post_counter: 0)
    user_comment.save
    comment = Comment.new(user: user_comment, post: post, text: "Hello @#{user.name}, it's interesting because....")
    comment.save
    update_counter = comment.update_counter
